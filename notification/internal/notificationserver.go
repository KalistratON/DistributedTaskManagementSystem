...
func parseMessageToTaskInfo(msg *kafka.Message) (NotificationTaskInfo, error) {
	var taskInfo NotificationTaskInfo
	if err := json.Unmarshal(msg.Value, &taskInfo); err != nil {
		log.Printf("error while trying parse message from kafka %v : %s", err, msg.Value)
		return NotificationTaskInfo{}, err
	}

	return taskInfo, nil
}

func getTaskLogInfoFromTaskInfo(clnTask *mongo.Collection, clnAuthor *mongo.Collection, taskInfo *NotificationTaskInfo) (NotificationTaskLogInfo, error) {
	var taskInfoLog NotificationTaskLogInfo
...
func pushToLog(taskInfo *NotificationTaskLogInfo) {
	log.Println("-------------")
...
func (service *NotificationService) Run() {
	...
	kafkaCh, closeFunc, err := helper.RunConsume(topic)
	if err != nil {
		log.Printf("%v", err)
		return
	}
